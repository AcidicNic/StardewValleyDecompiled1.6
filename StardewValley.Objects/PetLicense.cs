...
public class PetLicense : Object
{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, StackDrawType drawStackNumber, Color color, bool drawShadow)
	{
		...
		if (Game1.petData.ContainsKey(split[0]) && Game1.petData[split[0]].GetBreedById(split[1]) != null)
		{
			Rectangle sourceRect = Game1.petData[split[0]].GetBreedById(split[1]).IconSourceRect;
			spriteBatch.Draw(Game1.content.Load<Texture2D>(Game1.petData[split[0]].GetBreedById(split[1]).IconTexture), location + new Vector2(32f, 32f), sourceRect, color * transparency, 0f, new Vector2(sourceRect.Width / 2, sourceRect.Height / 2), 4f * drawnScale, SpriteEffects.None, layerDepth);
		}
