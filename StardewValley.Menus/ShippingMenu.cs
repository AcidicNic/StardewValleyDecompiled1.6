	...
	public override void draw(SpriteBatch b)
	{
			...
			foreach (ClickableTextureComponent c in categories)
			{
				if (introTimer < 2500 - i * 500)
				{
					...
					if (c.visible)
					{
						...
						categoryItems[i][0].drawInMenu(b, start + new Vector2(-88f, yOffset + 16), 1f, 1f, 0.9f, StackDrawType.Hide);
					}
					...
					categoryDials[i].draw(b, start + new Vector2(-itemSlotWidth - 192 - 48 + 4, 20f), categoryTotals[i]);
		...
		else
		{
			...
			for (int j = currentTab * itemsPerCategoryPage; j < currentTab * itemsPerCategoryPage + itemsPerCategoryPage; j++)
			{
				...
				{
					SpriteText.drawString(b, subtotalStr, (int)position.X + 64 + 12, (int)position.Y + 12);
					string dots = ".";
					for (int l = 0; l < boxwidth - 96 - SpriteText.getWidthOfString(subtotalStr + totalStr); l += SpriteText.getWidthOfString(" ."))
					{
						dots += " .";
					}
					SpriteText.drawString(b, dots, (int)position.X + 80 + SpriteText.getWidthOfString(subtotalStr), (int)position.Y + 8);
					SpriteText.drawString(b, totalStr, (int)position.X + boxwidth - 64 - SpriteText.getWidthOfString(totalStr), (int)position.Y + 12);
				}
				else
				{
					string dotsAndName = subtotalStr;
					int totalPosX = (int)position.X + boxwidth - 64 - SpriteText.getWidthOfString(totalStr);
					while (SpriteText.getWidthOfString(dotsAndName + totalStr) < boxwidth - 32)
					{
						dotsAndName += " .";
					}
