...
public class InventoryMenu : IClickableMenu
{
	...
	public override void draw(SpriteBatch b, int red = -1, int green = -1, int blue = -1)
	{
		for (int j = 0; j < inventory.Count; j++)
		{
			if (_iconShakeTimer.TryGetValue(j, out var endTime) && Game1.currentGameTime.TotalGameTime.TotalSeconds >= endTime)
			{
		...
		if (drawSlots)
		{
			for (int k = 0; k < capacity; k++)
			{
				...
				if (!Game1.options.gamepadControls && k < 12 && playerInventory)
				{
