...
public class Billboard : IClickableMenu
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (!dailyQuestBoard)
		{
			...
			for (int j = 0; j < calendarDays.Count; j++)
			{
				ClickableTextureComponent component = calendarDays[j];
				if (calendarDayData.TryGetValue(component.myID, out var day))
				{
					...
					{
						Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2(calendarDays[j].bounds.X + 8, (float)(calendarDays[j].bounds.Y + 60) - Game1.dialogueButtonScale / 2f), new Rectangle(103, 2, 10, 11), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 1f);
					}
