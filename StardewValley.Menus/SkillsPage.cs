		...
		: base(x, y, width, height)
	{
		...
		for (int k = 4; k < 10; k += 5)
		{
			for (int l = 0; l < 5; l++)
			{
				...
				if (drawRed && (k + 1) % 5 == 0)
				{
					skillBars.Add(new ClickableTextureComponent(professionNumber.ToString() ?? "", new Rectangle(addedX + drawX - 4 + k * 36, drawY + l * 68, 56, 36), null, professionBlurb, Game1.mouseCursors, new Rectangle(159, 338, 14, 9), 4f, drawShadow: true)
					{
