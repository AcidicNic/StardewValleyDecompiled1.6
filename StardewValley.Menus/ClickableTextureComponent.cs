...
public class ClickableTextureComponent : ClickableComponent
{
	...
	public virtual void draw(SpriteBatch b, Color c, float layerDepth, int frameOffset = 0, int xOffset = 0)
	{
		...
		if (texture != null)
		{
			...
			if (drawShadow)
			{
				Utility.drawWithShadow(b, texture, new Vector2((float)(bounds.X + xOffset) + (float)(sourceRect.Width / 2) * baseScale, (float)bounds.Y + (float)(sourceRect.Height / 2) * baseScale), r, c, 0f, new Vector2(sourceRect.Width / 2, sourceRect.Height / 2), scale, flipped: false, layerDepth);
			}
			else
			{
				b.Draw(texture, new Vector2((float)(bounds.X + xOffset) + (float)(sourceRect.Width / 2) * baseScale, (float)bounds.Y + (float)(sourceRect.Height / 2) * baseScale), r, c, 0f, new Vector2(sourceRect.Width / 2, sourceRect.Height / 2), scale, SpriteEffects.None, layerDepth);
			}
		}
		if (!string.IsNullOrEmpty(label))
		{
			...
			{
				Utility.drawTextWithShadow(b, label, Game1.smallFont, new Vector2(bounds.X + xOffset + bounds.Width, (float)bounds.Y + ((float)(bounds.Height / 2) - Game1.smallFont.MeasureString(label).Y / 2f)), Game1.textColor);
			}
			else
			{
				b.DrawString(Game1.smallFont, label, new Vector2(bounds.X + xOffset + bounds.Width, (float)bounds.Y + ((float)(bounds.Height / 2) - Game1.smallFont.MeasureString(label).Y / 2f)), Game1.textColor);
			}
		}
	}
