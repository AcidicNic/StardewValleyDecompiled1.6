...
public class DyeMenu : MenuWithInventory
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < _slotDrawPositions.Count; i++)
		{
			...
			{
				Color color2 = GetColorForPot(index);
				if (_hoveredPotIndex == -1 && HighlightItems(inventory.actualInventory[i]))
				{
					b.Draw(dyeTexture, _slotDrawPositions[i], new Rectangle(32, 96, 32, 32), color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.1f);
				}
			}
		}
		...
		for (int j = 0; j < dyePots.Count; j++)
		{
			...
			{
				Color color = GetColorForPot(j);
				b.Draw(dyeTexture, new Vector2(dyePots[j].bounds.X, dyePots[j].bounds.Y - 12), new Rectangle(_dyeDropAnimationFrames[j] / 50 * 16, 128, 16, 16), color, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.87f);
			}
			dyePots[j].draw(b);
		}
		if (!hoverText.Equals(""))
