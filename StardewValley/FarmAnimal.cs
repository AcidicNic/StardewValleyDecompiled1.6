...
public class FarmAnimal : Character
{
	...
	public void ReloadTextureIfNeeded(bool forceReload = false)
	{
		if (Sprite == null || forceReload)
		{
			...
			Sprite.textureUsesFlippedRightForLeft = data?.UseFlippedRightForLeft ?? false;
		}
		else
		{
	...
	public void pet(Farmer who, bool is_auto_pet = false)
	{
		if (!is_auto_pet)
		{
			...
			if (!hasEatenAnimalCracker.Value && who != null && who.ActiveObject != null && who.ActiveObject.QualifiedItemId == "(O)GoldenAnimalCracker")
			{
				if (type.Equals("Pig"))
				{
	...
	public virtual bool behaviors(GameTime time, GameLocation location)
	{
		...
		if ((bool)isEating)
		{
			...
			if (buildingTypeILiveIn.Contains("Barn") ? Sprite.Animate(time, 16, 4, 100f) : Sprite.Animate(time, 24, 4, 100f))
			{
				isEating.Value = false;
				Sprite.loop = true;
				Sprite.currentFrame = 0;
