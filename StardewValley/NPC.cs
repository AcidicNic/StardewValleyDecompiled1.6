...
public class NPC : Character, IComparable
{
	...
	public bool TryLoadSprites(string assetName, out string error, LocalizedContentManager content = null)
	{
		...
		if ((Sprite?.textureName.Value == assetName || Sprite?.Texture?.Name == assetName) && !Sprite.Texture.IsDisposed)
		{
			error = null;
			return true;
		}
	...
	public virtual bool tryToReceiveActiveObject(Farmer who, bool probe = false)
	{
		...
		case "(O)277":
			if (canReceiveGifts)
			{
				if (!probe)
				{
					if (!datable || friendship == null || !friendship.IsDating())
					{
	...
	public virtual void performTenMinuteUpdate(int timeOfDay, GameLocation location)
	{
		...
		{
			NPC obj2 = c as NPC;
			if ((obj2 == null || !obj2.SimpleNonVillagerNPC) && !data.FriendsAndFamily.ContainsKey(c.Name) && isFacingToward(c.Tile))
			{
				sayHiTo(c);
			}
		}
