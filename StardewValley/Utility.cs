...
public class Utility
{
	...
	/// <remarks>See also <see cref="M:StardewValley.Utility.ForEachVillager(System.Func{StardewValley.NPC,System.Boolean})" />.</remarks>
	public static void ForEachCharacter(Func<NPC, bool> action)
	{
		ForEachLocation(delegate(GameLocation location)
		{
			...
			{
				if (!current.eventActor && !action(current))
				{
					return false;
				}
			}
	...
	/// <param name="action">The action to perform for each character. This should return true (continue iterating) or false (stop).</param>
	/// <remarks>See also <see cref="M:StardewValley.Utility.ForEachCharacter(System.Func{StardewValley.NPC,System.Boolean})" />.</remarks>
	public static void ForEachVillager(Func<NPC, bool> action)
	{
		ForEachLocation(delegate(GameLocation location)
		{
			if (location is MovieTheater)
			...
			foreach (NPC current in location.characters)
			{
				if (!current.eventActor && current.IsVillager && !action(current))
				{
					return false;
				}
			}
	...
	public static void fixAllAnimals()
	{
		...
		ForEachLocation(delegate(GameLocation f)
		{
			...
			foreach (Building b in f.buildings)
			{
				...
				{
					animalHouse2.animalsThatLiveHere.RemoveWhere((long id) => getAnimal(id).home != b);
				}
			}
			...
			foreach (FarmAnimal current4 in allFarmAnimals)
			{
				...
				{
					current4.Position = recursiveFindOpenTileForCharacter(current4, f, new Vector2(40f, 40f), 200) * 64f;
	...
	/// <remarks>This creates a new list each time it's called, which is inefficient for hot paths. Consider using <see cref="M:StardewValley.Utility.ForEachCharacter(System.Func{StardewValley.NPC,System.Boolean})" /> if you don't need an actual list (e.g. you just need to iterate them once).</remarks>
	...
	/// <remarks>This creates a new list each time it's called, which is inefficient for hot paths. Consider using <see cref="M:StardewValley.Utility.ForEachVillager(System.Func{StardewValley.NPC,System.Boolean})" /> if you don't need an actual list (e.g. you just need to iterate them once).</remarks>
	...
	public static Item getTreasureFromGeode(Item geode)
	{
		...
		try
		{
			...
			Random r = CreateRandom(geodeId.Contains("MysteryBox") ? Game1.stats.Get("MysteryBoxesOpened") : Game1.stats.GeodesCracked, Game1.uniqueIDForThisGame / 2);
