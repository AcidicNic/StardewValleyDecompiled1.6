...
public class GameLocation : INetObject<NetFields>, IEquatable<GameLocation>, IAnimalLocation, IHaveModData
{
	...
	public virtual void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (critters != null)
		{
			for (int j = critters.Count - 1; j >= 0; j--)
			{
				if (critters[j].update(time, this))
				{
					critters.RemoveAt(j);
				}
			}
		...
		{
			int l = 0;
			while (l < projectiles.Count)
			{
				if (projectiles[l].update(time, this))
				{
		...
		for (int m = _activeTerrainFeatures.Count - 1; m >= 0; m--)
		{
			TerrainFeature feature = _activeTerrainFeatures[m];
		...
		{
			for (int n = largeTerrainFeatures.Count - 1; n >= 0; n--)
			{
				if (largeTerrainFeatures[n].tickUpdate(time))
				{
					largeTerrainFeatures.RemoveAt(n);
				}
			}
		}
		foreach (ResourceClump resourceClump in resourceClumps)
	...
	public void setUpLocationSpecificFlair()
	{
		...
		case "JoshHouse":
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Microsoft.Xna.Framework.Rectangle(386, 334, 36, 28), 40f, 3, 999999, new Vector2(246.5f, 317f) * 4f, flicker: false, flipped: false, 0.13601f, 0f, Color.White, 2f, 0f, 0f, 0f));
			}
			break;
		case "LewisBasement":
		{
	...
	public virtual bool blacksmith(Location tileLocation)
	{
		foreach (NPC i in characters)
		{
			...
			{
				Game1.DrawDialogue(i, "Data\\ExtraDialogue:Clint_StillWorking", Game1.player.toolBeingUpgraded.Value.DisplayName);
			}
			return true;
		}
		return false;
	...
	public virtual bool answerDialogueAction(string questionAndAnswer, string[] questionParams)
	{
		...
		case "professionForget_mining":
		{
			...
			RemoveProfession(20);
			Game1.drawObjectDialogue(Game1.content.LoadString("Strings\\Locations:Sewer_DogStatueFinished"));
			int num4 = Farmer.checkForLevelGain(0, Game1.player.experiencePoints[3]);
			if (num4 >= 5)
			{
	...
	internal static Item GetFishFromLocationData(string locationName, Vector2 bobberTile, int waterDepth, Farmer player, bool isTutorialCatch, bool isInherited, GameLocation location, ItemQueryContext itemQueryContext)
	{
		...
		if (player?.CurrentTool is FishingRod rod)
		{
			...
			if (bait != null && bait.Name.Contains("Bait") && bait.preservedParentSheetIndex.Value != null)
			{
	...
	{
		return GetData((this is MineShaft) ? "UndergroundMine" : Name);
	}
	...
	public virtual void spawnObjects()
	{
		...
		while (r.NextDouble() < chanceForNewArtifactAttempt)
		{
			...
			if (CanItemBePlacedHere(location) && !IsTileOccupiedBy(location) && getTileIndexAt(xCoord, yCoord, "AlwaysFront") == -1 && getTileIndexAt(xCoord, yCoord, "Front") == -1 && !isBehindBush(location) && (doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null || (GetSeason() == Season.Winter && doesTileHaveProperty(xCoord, yCoord, "Type", "Back") != null && doesTileHaveProperty(xCoord, yCoord, "Type", "Back").Equals("Grass"))))
			{
				...
				objects.Add(location, ItemRegistry.Create<Object>(r.NextBool(0.25) ? "(O)SeedSpot" : "(O)590"));
			}
	...
	public void BuildStartingCabins()
	{
		if (_startingCabinLocations.Count > 0)
		{
			...
			{
				removeObjectsAndSpawned((int)_startingCabinLocations[i].X, (int)_startingCabinLocations[i].Y, 5, 3);
				removeObjectsAndSpawned((int)_startingCabinLocations[i].X + 2, (int)_startingCabinLocations[i].Y + 3, 1, 1);
				Building b = new Building("Cabin", _startingCabinLocations[i]);
				b.magical.Value = true;
				b.skinId.Value = cabinStyleOrder[i % cabinStyleOrder.Count];
				b.daysOfConstructionLeft.Value = 0;
				b.load();
				buildStructure(b, _startingCabinLocations[i], Game1.player, skipSafetyChecks: true);
