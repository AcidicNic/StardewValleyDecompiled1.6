...
public class Farm : GameLocation
{
	...
	public void onNewGame()
	{
		...
		else if (Game1.whichFarm == 7 && Game1.whichModFarm.Id == "MeadowlandsFarm")
		{
			...
			{
				objects.Add(new Vector2(47f, y2), new Fence(new Vector2(47f, y2), "322", isGate: false));
			}
			for (int y = 7; y < 20; y++)
			{
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		{
			if (characters[j] is Pet pet && (getTileIndexAt(pet.TilePoint, "Buildings") != -1 || getTileIndexAt(pet.TilePoint.X + 1, pet.TilePoint.Y, "Buildings") != -1 || !CanSpawnCharacterHere(pet.Tile) || !CanSpawnCharacterHere(new Vector2(pet.TilePoint.X + 1, pet.TilePoint.Y))))
			{
				pet.WarpToPetBowl();
			}
		}
		...
		if (Game1.whichFarm == 5)
		{
			...
			if (objects.Length > 0)
			{
				for (int l = 0; l < 6; l++)
				{
					if (Utility.TryGetRandom(objects, out key, out var o2) && o2.IsWeeds() && o2.tileLocation.X < 36f && o2.tileLocation.Y < 34f)
					{
				...
				if (doesTileHavePropertyNoNull((int)v2.X, (int)v2.Y, "BeachSpawn", "Back") != "")
				{
					...
					if (Game1.stats.DaysPlayed > 1)
					{
						if (Game1.random.NextDouble() < 0.15 || Game1.stats.Get("beachFarmSpawns") % 4 == 0)
						{
							...
							objects.Add(v2, new Object(whichItem, 1)
							{
								Fragility = 2,
								MinutesUntilReady = 3
							});
	...
	protected override void resetLocalState()
	{
		...
		for (int j = characters.Count - 1; j >= 0; j--)
		{
			if (characters[j] is Child child)
			{
