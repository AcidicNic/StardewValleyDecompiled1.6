...
public class Event
{
	...
	public static class DefaultCommands
	{
		...
		public static void LoadActors(Event @event, string[] args, EventContext context)
		{
			...
			if (@event.festivalData != null)
			{
				...
				{
					string[] array = SplitCommands(data);
		...
		: this()
	{
		...
		id = eventID;
		eventCommands = SplitCommands(eventString);
	...
	public static bool tryToLoadFestival(string festival, out Event ev)
	{
		...
		ev.eventCommands = SplitCommands(rawSetUp);
	...
	private void addSpecificTemporarySprite(string key, GameLocation location, string[] args)
	{
		...
		case "getEndSlideshow":
		{
			...
			string[] s = SplitCommands(obj.getEndSlideshow());
	...
	public void startSecretSantaEvent()
	{
		...
		eventCommands = SplitCommands(rawCommands);
		...
		if (festivalTimer > 0)
		{
			...
			if (festivalTimer <= 0)
			{
				...
				else
				{
					...
					eventCommands = SplitCommands(rawCommands2);
					currentCommand = 0;
				}
			}
		}
	...
	public void forceFestivalContinue()
	{
		...
		string[] newCommands = SplitCommands(rawCommands);
	...
	public void setUpFestivalMainEvent()
	{
		...
		List<string> commandsToAdd = new List<string>(SplitCommands(rawFestivalData));
		...
		eventCommands = SplitCommands(rawFestivalData);
	}
	...
	public bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		if (isFestival)
		{
			...
			{
			case 958:
				if (isSpecificFestival("fall27") && tileLocation.X == 61 && tileLocation.Y == 13 && who.IsLocalPlayer)
				{
