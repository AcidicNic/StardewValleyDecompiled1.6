...
public class GameRunner : Game
{
	...
	protected override void Update(GameTime gameTime)
	{
		...
		for (int k = 0; k < activeNewDayProcesses.Count; k++)
		{
			KeyValuePair<Game1, IEnumerator<int>> active_new_days = activeNewDayProcesses[k];
			Game1 instance3 = activeNewDayProcesses[k].Key;
			LoadInstance(instance3);
			if (!active_new_days.Value.MoveNext())
			{
				instance3.isLocalMultiplayerNewDayActive = false;
				activeNewDayProcesses.RemoveAt(k);
				k--;
				Utility.CollectGarbage();
			}
			SaveInstance(instance3);
		}
		...
		for (PlayerIndex player_index2 = PlayerIndex.One; player_index2 <= PlayerIndex.Four; player_index2++)
		{
			...
			{
				if (startButtonState[(int)player_index2] >= 0)
				{
					startButtonState[(int)player_index2]++;
				}
			}
			else
			{
				startButtonState[(int)player_index2] = 0;
			}
		}
		for (int j = 0; j < gameInstances.Count; j++)
		{
			...
			if (j == 0)
			{
				...
				for (PlayerIndex player_index = start_player_index; player_index <= PlayerIndex.Four; player_index++)
				{
					...
					if (!fail && instance2.IsLocalCoopJoinable() && IsStartDown(player_index) && instance2.ShowLocalCoopJoinMenu())
					{
