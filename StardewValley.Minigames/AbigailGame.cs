...
public class AbigailGame : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		if (gopherRunning)
		{
			...
			{
				if (gopherBox.Intersects(monsters[k].position))
				{
		...
		for (int l = temporarySprites.Count - 1; l >= 0; l--)
		{
			if (temporarySprites[l].update(time))
			{
				temporarySprites.RemoveAt(l);
			}
		}
		if (motionPause <= 0)
		{
			for (int n = powerups.Count - 1; n >= 0; n--)
			{
				if (Utility.distance(playerBoundingBox.Center.X, powerups[n].position.X + TileSize / 2, playerBoundingBox.Center.Y, powerups[n].position.Y + TileSize / 2) <= (float)(TileSize + 3) && (powerups[n].position.X < TileSize || powerups[n].position.X >= 16 * TileSize - TileSize || powerups[n].position.Y < TileSize || powerups[n].position.Y >= 16 * TileSize - TileSize))
				{
					if (powerups[n].position.X + TileSize / 2 < playerBoundingBox.Center.X)
					{
						powerups[n].position.X++;
					}
					if (powerups[n].position.X + TileSize / 2 > playerBoundingBox.Center.X)
					{
						powerups[n].position.X--;
					}
					if (powerups[n].position.Y + TileSize / 2 < playerBoundingBox.Center.Y)
					{
						powerups[n].position.Y++;
					}
					if (powerups[n].position.Y + TileSize / 2 > playerBoundingBox.Center.Y)
					{
						powerups[n].position.Y--;
				...
				powerups[n].duration -= time.ElapsedGameTime.Milliseconds;
				if (powerups[n].duration <= 0)
				{
					powerups.RemoveAt(n);
				}
			}
			for (int i2 = activePowerups.Count - 1; i2 >= 0; i2--)
			{
				int key = activePowerups.ElementAt(i2).Key;
			...
			if (deathTimer <= 0f && playerMovementDirections.Count > 0 && !scrollingMap)
			{
				...
				for (int i4 = 0; i4 < runSpeedLevel; i4++)
				{
					speed *= 1.25f;
				}
				for (int i5 = Math.Max(0, playerMovementDirections.Count - 2); i5 < playerMovementDirections.Count; i5++)
				{
					if (i5 != 0 || playerMovementDirections.Count < 2 || playerMovementDirections.Last() != (playerMovementDirections.ElementAt(playerMovementDirections.Count - 2) + 2) % 4)
					{
						Vector2 newPlayerPosition = playerPosition;
						switch (playerMovementDirections.ElementAt(i5))
						{
						case 0:
			...
			if (shopping)
			{
				...
				else if (merchantShopOpen)
				{
					...
					{
						KeyValuePair<Rectangle, int> pair = storeItems.ElementAt(i8);
			...
			if (deathTimer <= 0f && playerShootingDirections.Count > 0 && shotTimer <= 0)
			{
				...
				for (int i6 = 0; i6 < fireSpeedLevel; i6++)
				{
			...
			else if (deathTimer <= 0f && !waitingForPlayerToMoveDownAMap && !shopping && !shootoutLevel)
			{
				...
				for (int p = 0; p < 4; p++)
				{
					...
					case 1:
					{
						for (int y2 = 7; y2 < 10; y2++)
						{
							...
							{
								CowboyMonster monster4 = new CowboyMonster(spawnQueue[p][0].X, new Point(15 * TileSize, y2 * TileSize));
								if (whichRound > 0)
								{
					...
					case 2:
					{
						for (int x = 7; x < 10; x++)
						{
							...
							{
								CowboyMonster monster3 = new CowboyMonster(spawnQueue[p][0].X, new Point(x * TileSize, 15 * TileSize));
								if (whichRound > 0)
								{
				...
				if (waveTimer <= 0 && monsters.Count == 0 && isSpawnQueueEmpty())
				{
					...
					if (whichRound > 0)
					{
						for (int i3 = 0; i3 < monsterChances.Count; i3++)
						{
							_ = monsterChances[i3];
							monsterChances[i3] *= 1.1f;
						}
					}
			...
			for (int m = monsters.Count - 1; m >= 0; m--)
			{
				monsters[m].move(playerPosition, time);
				if (m < monsters.Count && monsters[m].position.Intersects(playerBoundingBox) && playerInvincibleTimer <= 0)
				{
					if (zombieModeTimer <= 0)
					{
						playerDie();
				...
				if (playingWithAbigail && m < monsters.Count && monsters[m].position.Intersects(player2BoundingBox) && player2invincibletimer <= 0)
				{
					Game1.playSound("Cowboy_monsterDie");
	...
	public void draw(SpriteBatch b)
	{
		...
		else if (endCutscene)
		{
			...
			case 3:
			{
				for (int j = 0; j < 16; j++)
				{
					for (int m = 0; m < 16; m++)
					{
						b.Draw(Game1.mouseCursors, topLeftScreenCoordinate + new Vector2(j, m) * 16f * 3f + new Vector2(0f, newMapPosition - 16 * TileSize), new Rectangle(464 + 16 * map[j, m] + ((map[j, m] == 5 && cactusDanceTimer > 800f) ? 16 : 0), 1680 - world * 16, 16, 16), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 0f);
					}
				}
		...
		else
		{
			...
			else
			{
				...
				for (int k = 0; k < whichWave + whichRound * 12; k++)
				{
					b.Draw(Game1.mouseCursors, topLeftScreenCoordinate + new Vector2(TileSize * 16 + 3, k * 3 * 6), new Rectangle(512, 1760, 5, 5), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 0.5f);
				}
