...
public class MineCart : IMinigame
{
		...
		if (gameState == GameStates.Ingame)
		{
			...
			if (checkpointPositions.Count > 0)
			{
				...
				while (k < checkpointPositions.Count && player.position.X >= (float)(checkpointPositions[k] * tileSize))
				{
					foreach (Entity entity2 in _entities)
					{
						if (entity2 is CheckpointIndicator indicator2 && (int)(indicator2.position.X / (float)tileSize) == checkpointPositions[k])
						{
							indicator2.Activate();
							break;
						}
	...
	public void ShowMap()
	{
		...
		else if (currentTheme == 3)
		{
			...
			for (int m = 0; m < 32; m++)
			{
				AddEntity(new MineDebris(new Rectangle(51, 53, 9, 9), new Vector2(250f, 136f) + new Vector2(Game1.random.Next(-20, 31), Game1.random.Next(-20, 21)), Game1.random.Next(-30, 31), Game1.random.Next(-70, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, holdLastFrame: false, 0.8f + 0.01f * (float)m));
			}
		}
		...
		else if (currentTheme == 5)
		{
			...
			for (int n = 0; n < 8; n++)
			{
				AddEntity(new MineDebris(new Rectangle(427, 607, 6, 6), new Vector2(205 + Game1.random.Next(3, 14), 26 + Game1.random.Next(6, 14)), Game1.random.Next(-30, 31), Game1.random.Next(-60, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, holdLastFrame: false, 5f + (float)n * 0.005f));
			}
		}
		if (currentTheme == 9)
		{
		...
		else if (currentTheme == 6)
		{
			for (int j = 0; j < 52; j++)
			{
				AddEntity(new CosmeticFallingBoulder(Game1.random.Next(72, 195), new Color(100, 66, 49), 96 + Game1.random.Next(-10, 11), 0.65f + (float)j * 0.05f)).position = new Vector2(5 + Game1.random.Next(360), -16f);
			}
		}
	...
	public void draw(SpriteBatch b)
	{
		...
		if (gameState != GameStates.Map)
		{
			...
			{
				for (int m = 0; m <= screenWidth / tileSize + 1; m++)
				{
					b.Draw(texture, TransformDraw(new Rectangle(m * tileSize - (int)lakeSpeedAccumulator % tileSize, tileSize * 9, tileSize, screenHeight - 96)), lakeBGSource, lakeTint, 0f, Vector2.Zero, SpriteEffects.None, 0.9f);
				}
				for (int n = 0; n < lakeDecor.Count; n++)
				{
					lakeDecor[n].Draw(b);
				}
				for (int i2 = 0; i2 <= screenWidth / backBGSource.Width + 2; i2++)
				{
					b.Draw(texture, TransformDraw(new Vector2(0f - backBGPosition + (float)(i2 * backBGSource.Width), backBGYOffset)), backBGSource, backBGTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.7f);
				}
		...
		if (gameState == GameStates.Title)
		{
			...
			if (gameMode == 2)
			{
				...
				for (int i4 = 0; i4 < 5 && i4 < _currentHighScores.Count; i4++)
				{
					...
					KeyValuePair<string, int> score = _currentHighScores[i4];
					int score_text_width = (int)Game1.dialogueFont.MeasureString(score.Value.ToString() ?? "").X / 4;
					b.DrawString(Game1.dialogueFont, "#" + (i4 + 1), TransformDraw(draw_position3), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
					...
					b.DrawString(Game1.dialogueFont, "#" + (i4 + 1), TransformDraw(draw_position3 + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
		...
		else if (gameState != GameStates.FruitsSummary)
		{
			...
			else
			{
				...
				{
					for (int i5 = -2; i5 <= screenHeight / tileSize + 1; i5++)
					{
						b.Draw(texture, TransformDraw(new Vector2((float)(screenWidth + tileSize * j2) - waterFallPosition, i5 * tileSize + (int)(_totalTime * 48.0 + (double)(tileSize * 100)) % tileSize)), new Rectangle(48, 32, 16, 16), waterfallTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.2f);
					}
				}
			}
		}
