...
public class Quest : INetObject<NetFields>, IQuest, IHaveModData
{
		...
		get
		{
			if (!_loadedTitle)
			{
				...
				case 3:
					...
					{
						_questTitle = Game1.content.LoadString("Strings\\1_6_Strings:ItemDeliveryQuestTitle", NPC.GetDisplayName(quest4.target.Value));
					}
					else
					{
				...
				case 4:
					...
					{
						_questTitle = Game1.content.LoadString("Strings\\1_6_Strings:MonsterQuestTitle", Monster.GetDisplayName(quest3.monsterName.Value));
					}
					else
					{
	...
	public static Quest getQuestFromId(string id)
	{
		...
		case "ItemDelivery":
		{
			if (!TryParseConditions(fields, out var conditions5, out error) || !ArgUtility.TryGet(fields, 9, out var targetMessage, out error, allowBlank: false))
			{
				return LogParseError(id, error);
			}
