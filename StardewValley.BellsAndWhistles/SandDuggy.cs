...
public class SandDuggy : INetObject<NetFields>
{
	...
	public virtual int FindRandomFreePoint()
	{
		...
		{
			Point hole_location2 = holeLocations[j];
			if (!locationRef.Value.isObjectAtTile(hole_location2.X, hole_location2.Y) && !locationRef.Value.isTerrainFeatureAt(hole_location2.X, hole_location2.Y) && !locationRef.Value.terrainFeatures.ContainsKey(Utility.PointToVector2(hole_location2)))
			{
