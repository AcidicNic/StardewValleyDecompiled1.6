...
public class Tree : TerrainFeature
{
	...
	{
		if (Game1.IsFall && Game1.random.NextDouble() < 0.05 && !tapped && (treeType.Value == "1" || treeType.Value == "2") && Location != null && !(Location is Town) && !Location.IsGreenhouse)
		{
			if (treeType == "1")
			{
				treeType.Value = "10";
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if ((int)growthStage >= 5)
		{
			if ((bool)hasMoss)
			{
				...
				Game1.createMultipleItemDebris(ItemRegistry.Create("(O)Moss", rand.Next(1, 3)), new Vector2(tileLocation.X, tileLocation.Y - 1f) * 64f, -1, location, Game1.player.StandingPixel.Y - 32);
