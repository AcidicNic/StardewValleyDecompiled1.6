...
public class DesertFestival : Desert
{
	...
	public static void SetupMerchantSchedule(NPC character, int shop_index)
	{
		...
		schedule.Remove(0, 1);
		Game1.warpCharacter(character, character.DefaultMap, new Point((int)(character.DefaultPosition.X / 64f), (int)(character.DefaultPosition.Y / 64f)));
	...
	public override bool performAction(string action, Farmer who, Location tile_location)
	{
		...
		case "DesertGil":
			...
			if (highestCalicoEggRatingToday == 0)
			{
				Game1.DrawDialogue((Game1.getLocationFromName("AdventureGuild") as AdventureGuild).Gil, "Strings\\1_6_Strings:Gil_NoRating");
				return true;
			}
			createQuestionDialogue(Game1.content.LoadString("Strings\\1_6_Strings:Gil_SubmitRating", highestCalicoEggRatingToday + 1), createYesNoResponses(), "Gil_EggRating");
			return true;
		case "DesertMarlon":
		{
